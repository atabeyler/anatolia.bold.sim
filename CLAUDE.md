...
convenience, not a write-only-secret workaround). If this repo's visibility
or collaborator list ever changes, move this out first — and note that
several of these (the third-party API keys, `GITHUB_RELEASES_TOKEN`,
`JWT_SECRET`/`JWT_REFRESH_SECRET`) are trivially rotatable, unlike the
keystore, so there is comparatively less reason to keep them here long-term.
...
CLIENT_URL=https://anatolia-sim-client.onrender.com
DATABASE_URL=postgresql://anatolia_dbs_user:32Z0K95apzR2rpL7IS6hc2v1VQ4CiiUx@dpg-d9jfrqmrnols738mcf6g-a/anatolia_dbs
