...
/// Self-service counterpart to `admin_update_user`: same editable field set
/// minus `role` (a regular user can never grant themselves admin this way)
/// and minus any admin-permission check at the call site (the caller in
/// auth.rs derives `id` from the requester's own JWT claims, never a
/// client-supplied id, so this can only ever touch the caller's own row).
#[allow(clippy::too_many_arguments)]
