...
// RENDER_EXTERNAL_URL is the same signal AppState::new() already uses to
// detect "this is the Render deployment" -- checked here directly (rather
// than deriving it from the DB backend) since this runs before
// AppState::new() decides that.
/// A real report from a 1-core Render instance showed `cpu_cores_used: 2`
/// next to `cpu_cores_available: 1` -- forcing a second rayon worker thread
/// when there's only one real core to run on buys zero parallelism and pure
/// thread-contention overhead instead. `available_parallelism` never forces
/// *more* threads than the container itself has, only ever caps at 2 on
/// Render.
...
async fn run() -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
    ...
    let mut app = Router::new()
        ...
        .nest("/api/admin", Router::new()
            ...
            .route("/users/:id", axum::routing::put(admin::update_user_details).delete(admin::delete_user_route))
